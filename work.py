...
def create_window():
    ...
    global number
    type_of_view = True
    very_massive = con.extract("Work")
    number = 0
    main_window = t.Tk()
    main_window.title("Карточка услуг")
    ...
    def show_tree():
        ...
        tree = ttk.Treeview(
            center_frame,
            columns=("UID", "Name", "Cost"),
            show="headings",
            selectmode="browse",
        )
        tree.grid(row=3)
        tree.heading("UID", text="Уникальный идентификатор")
        tree.heading("Name", text="Название услуги")
        tree.heading("Cost", text="Стоимость услуги")
        ...
        def change_info():
            con.change_wrk((uid_entry.get(), name_entry.get(), cost_entry.get()))
            global very_massive
            very_massive = con.extract("Work")
            update_tree()

        def add_info():
            con.create_wrk((uid_entry.get(), name_entry.get(), cost_entry.get()))
            global very_massive
            very_massive = con.extract("Work")
            update_tree()

        def delete_info():
            con.delete("Work", uid_entry.get())
            global very_massive
            very_massive = con.extract("Work")
            update_tree()
    ...
    def show_form():
        ...
        name_label.configure(text="Название услуги")

        cost_label = t.Label(
            center_frame,
        )
        cost_label.grid(row=3, column=1, pady=5)
        cost_label.configure(text="Стоимость услуги")
