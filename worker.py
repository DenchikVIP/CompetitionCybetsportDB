...
def create_window():
    ...
    global number
    type_of_view = True
    very_massive = con.extract("Worker")
    print(very_massive)
    number = 0
    main_window = t.Tk()
    main_window.title("Карточка рабочего")
    ...
    def show_tree():
        ...
        tree = ttk.Treeview(
            center_frame,
            columns=("UID", "FIO", "Phone", "Assignment 1", "Assignment 2", "Assignment 3"),
            show="headings",
            selectmode="browse",
        )
        tree.grid(row=3)
        ...
        tree.heading("Assignment 1", text="Специальность 1")
        tree.heading("Assignment 2", text="Специальность 2")
        tree.heading("Assignment 3", text="Специальность 3")
        ...
        assignment1_entry = t.Entry(entry_frame)
        assignment1_entry.grid(column=4, row=1)
        assignment2_entry = t.Entry(entry_frame)
        assignment2_entry.grid(column=5, row=1)
        assignment3_entry = t.Entry(entry_frame)
        assignment3_entry.grid(column=6, row=1)
        ...
        def row_pick(event):
            ...
            assignment1_entry.delete(0, t.END)
            assignment1_entry.insert(0, tree.item(tree.selection()[0])["values"][3])
            assignment2_entry.delete(0, t.END)
            assignment2_entry.insert(0, tree.item(tree.selection()[0])["values"][4])
            assignment3_entry.delete(0, t.END)
            assignment3_entry.insert(0, tree.item(tree.selection()[0])["values"][5])

        tree.bind("<<TreeviewSelect>>", row_pick)

        def change_info():
            con.change_wrkr((uid_entry.get(), fio_entry.get(), phone_entry.get(), assignment1_entry.get(),
                             assignment2_entry.get(), assignment3_entry.get()))
            global very_massive
            very_massive = con.extract("Worker")
            update_tree()

        def add_info():
            con.create_wrkr((uid_entry.get(), fio_entry.get(), phone_entry.get(), assignment1_entry.get(),
                             assignment2_entry.get(), assignment3_entry.get()))
            global very_massive
            very_massive = con.extract("Worker")
            update_tree()
        ...
        def delete_info():
            con.delete("Worker", uid_entry.get())
            global very_massive
            very_massive = con.extract("Worker")
            update_tree()
    ...
    def show_form():
        ...
        uid_label.configure(text="UID Рабочего")

        fio_label = t.Label(
            center_frame,
        )
        fio_label.grid(row=2, column=1, pady=5)
        fio_label.configure(text="Ф.И.О. Рабочего")

        phone_label = t.Label(
            center_frame,
        )
        phone_label.grid(row=3, column=1, pady=5)
        phone_label.configure(text="Номер телефона Рабочего")

        assgnment1_label = t.Label(
            center_frame,
        )
        assgnment1_label.grid(row=4, column=1, pady=5)
        assgnment1_label.configure(text="Специальность №1")

        assgnment2_label = t.Label(
            center_frame,
        )
        assgnment2_label.grid(row=5, column=1, pady=5)
        assgnment2_label.configure(text="Специальность №2")

        assgnment3_label = t.Label(
            center_frame,
        )
        assgnment3_label.grid(row=6, column=1, pady=5)
        assgnment3_label.configure(text="Специальность №3")
        ...
        assignment1_entry = t.Entry(
            center_frame,
        )
        assignment1_entry.grid(row=4, column=2, pady=5)
        assignment1_entry.insert(0, very_massive[number][3])

        assignment2_entry = t.Entry(
            center_frame,
        )
        assignment2_entry.grid(row=5, column=2, pady=5)
        assignment2_entry.insert(0, very_massive[number][4])

        assignment3_entry = t.Entry(
            center_frame,
        )
        assignment3_entry.grid(row=6, column=2, pady=5)
        assignment3_entry.insert(0, very_massive[number][5])
